...
- **角色权限控制**：三种角色（admin/user/visitor），visitor 登录后仅提示无权限，admin 可选择任意身份登录。
...
- **数据分析 Agent**：三工具流水线（list_files → inspect_file → execute_data_query），自然语言转 Python 代码后在沙箱执行。
- **文档编写 Agent**：四工具组合（知识库检索 → 目录生成 → 文档内容生成 → 目录修改），支持逐章节分段生成降低单次请求压力。
- **直接调用模式**：chat_direct() 绕过 Router 直接指定 Sub-Agent，用于数据统计和文档编写等需要精确控制场景。
- **工具调用日志**：记录 Agent 调用的工具名称、参数和返回结果摘要，可展开查看推理过程，transfer 类内部转发已过滤。

## 四、RAG 文档功能

- **查询改写**：LLM 将口语化问题改写为适合向量语义检索的形式，输出 rewritten_query + keywords + sub_questions，短问题（≤10字或≤3词）自动跳过改写。
...
- **三步式向导 UI**：流程步骤指示器（输入需求 → 确认目录 → 生成文档），支持嵌入首页或独立页面两种模式。
...
- **自动保存与对话同步**：文档生成后自动存入 document_history 表，同时创建侧边栏对话记录便于回溯。
...
- **多阶段检查**：输入阶段（check_user_input）防提示词注入，代码阶段（check_generated_code）拦截危险操作，答案阶段（check_generated_answer）做质量校验。
...
- **细粒度操作类型**：区分 chat、doc_outline_generate、doc_generate、update_password、update_display_name 等多种操作。

## 十一、LLM 与可观测性

- **主模型配置**：阿里千问 qwen-plus，temperature=0，max_tokens=8192，timeout=300s，max_retries=2。
- **本地 Ollama 备选**：代码保留 Ollama 本地模型接入配置（qwen3.6:35b），注释切换即可启用。
